...
import { CATNFT_CONTRACT_ADDRESS } from '@/config';
import { Spinner } from '@radix-ui/themes';
import { CircleCheckBig, CircleX, Contact, Globe, LogOut, MousePointerClick, Pickaxe, Shapes, TriangleAlert, Wallet } from 'lucide-react';
...
const MintCatNFT = () => {
    ...
    const [catType, setCatType] = useState<string>(searchParams.get("catType") || "")
    ...
    useEffect(() => {
        if (tokensOfOwner) {
            const data = convertBigIntJsonToString(tokensOfOwner)
            console.log("🚀 ~ useEffect ~ tokensOfOwner:", convertToArray(data))
        }
    }, [tokensOfOwner])


    // useEffect(() => {
    //     if (isSuccess && signMessageData && address) {
    //         verifySignature(signMessageData, userID, address, chainId)
    //     }
    // }, [signMessageData, isSuccess])


    const { data: hash, isPending: isWriteContracting, writeContract, error } = useWriteContract();

    const { data: hashClaim, isPending: isWriteContractingClaim, writeContract: writeContractClaim, error: errorClaim } = useWriteContract();
    ...
    const handleClickMetamask = () => {
        if (userID && catType && chainId) {
            const url = `https://metamask.app.link/dapp/meowtopia-wallet-connect.vercel.app/mint-cat-nft?catType=${catType}&userID=${userID}&chainId=${chainId}`
            window.open(url)
        }
    }
    ...
    const handClickOKX = () => {
        if (userID && catType && chainId) {
            const url = `https://meowtopia-wallet-connect.vercel.app/mint-cat-nft?catType=${catType}&userID=${userID}&chainId=${chainId}`
    ...
    const handleClaimToken = async () => {
        ...
        console.log("🚀 ~ handleClaimToken ~ hashClaim:", hashClaim)
    }
    ...
    const handleClickBinance = () => {
        if (userID && catType && chainId) {
            const url = encodeURIComponent(`https://meowtopia-wallet-connect.vercel.app/mint-cat-nft?catType=${catType}&userID=${userID}&chainId=${chainId}`)
    ...
    return (
        <div className="relative w-full px-[1rem] pb-[2vh] min-h-screen flex flex-col items-center justify-start gap-y-[40px] bg-[url('@/assets/bg.png')] bg-cover bg-center bg-no-repeat font-Rubik">
            ...
            <div className="w-full flex flex-col gap-4 max-w-[500px]">
                ...
                {
                    isConfirmed && <div className='text-green-500 font-bold p-2 rounded-lg border-2 border-black shadow-lg flex items-center justify-center bg-white'><CircleCheckBig className='mr-2' />Minting NFT success. Check your item in Meowtopia</div>
                }
                <Card className='border-2 border-black shadow-lg'>
                    <CardHeader>
                        <CardTitle className='justify-center text-[16px] break-words flex items-center gap-x-2 mb-2'><Pickaxe color='green' size={25} />MINT CAT NFT</CardTitle>
                        {userID && <div className='text-black font-bold flex items-center gap-x-2'><Contact color='green' size={25} /> User ID: {userID}</div>}
                        {catType && <div className='text-black font-bold flex items-center gap-x-2'><Shapes color='green' size={25} /> Cat Type: {catType}</div>}
                        {isConnected && catType && userID && chainId === chain?.id && <div className="w-full flex items-center justify-center gap-x-2">
                            <Button disabled={isWriteContracting || isConfirming} onClick={handleMintCatNFT} className={`${isConfirming ? 'w-full active:translate-y-1 bg-green-800 mt-3' : 'active:translate-y-1 bg-[#f68329] mt-3 hover:animate-none hover:bg-green-900 border-2 border-black text-black hover:text-white flex items-center justify-center shadow-lg w-fit duration-200'}`}>
                                {!isConfirming && <MousePointerClick className='mr-2' />}{isConfirming ? 'Confirming...' : 'MINT NOW'}
                            </Button>
                            <Button disabled={isWriteContractingClaim || isConfirmingClaim} onClick={handleClaimToken} className={`${isConfirmingClaim ? 'w-full active:translate-y-1 bg-green-800 mt-3' : 'active:translate-y-1 bg-[#f68329] mt-3 hover:animate-none hover:bg-green-900 border-2 border-black text-black hover:text-white flex items-center justify-center shadow-lg w-fit duration-200'}`}>
                                {!isConfirmingClaim && <MousePointerClick className='mr-2' />}{isConfirmingClaim ? 'Claiming ...' : 'CLAIM TOKEN'}
                            </Button>
                        </div>
                        }
                    </CardHeader>
