...
import MaterialNFTABI from "@/abi/MaterialNFT.json";
...
const EquipItemNFT = () => {
    ...
    const [catId, setCatId] = useState<string>(searchParams.get("catId") || "")
    ...
    const { data: isApproved } = useReadContract({
        ...
        args: [address, CATNFT_CONTRACT_ADDRESS],
    })

    useEffect(() => {
        console.log("🚀 ~ EquipItemNFT ~ isApproved:", isApproved)
    }, [isApproved])

    const { data: hashApprove, isPending: isWriteContractingApprove, writeContract: writeContractApprove, error: errorApprove } = useWriteContract();
    ...
    const handleApprove = async () => {
        writeContractApprove({
            ...
            args: [CATNFT_CONTRACT_ADDRESS, true],
        });
    }

    const handleClickMetamask = () => {
        if (userID && catId && itemId && chainId) {
            const url = `https://metamask.app.link/dapp/meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?catId=${catId}&itemId=${itemId}&&userID=${userID}&chainId=${chainId}`
            window.open(url)
        }
    }
    ...
    const handClickOKX = () => {
        if (userID && catId && itemId && chainId) {
            const url = `https://meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?catId=${catId}&itemId=${itemId}&&userID=${userID}&chainId=${chainId}`
    ...
    const handleClickBinance = () => {
        if (userID && catId && itemId && chainId) {
            const url = encodeURIComponent(`https://meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?catId=${catId}&itemId=${itemId}&&userID=${userID}&chainId=${chainId}`)
    ...
    return (
        <div className="relative w-full px-[1rem] pb-[2vh] min-h-screen flex flex-col items-center justify-start gap-y-[40px] bg-[url('@/assets/bg.png')] bg-cover bg-center bg-no-repeat font-Rubik">
            ...
            <div className="w-full flex flex-col gap-4 max-w-[500px]">
                ...
                {errorApprove && (
                    <div className='text-red-500 font-bold p-2 rounded-lg border-2 border-black shadow-lg flex items-center justify-center bg-white'><CircleX className='mr-2' />Error: {(errorApprove as BaseError).shortMessage || errorApprove.message}</div>
                )}
                {
                    isConfirmed && <div className='text-green-500 font-bold p-2 rounded-lg border-2 border-black shadow-lg flex items-center justify-center bg-white'><CircleCheckBig className='mr-2' />Equip Item success. Check your item in Meowtopia</div>
                }
                <Card className='border-2 border-black shadow-lg'>
                    <CardHeader>
                        <CardTitle className='justify-center text-[16px] break-words flex items-center gap-x-2 mb-2'><ShieldPlus color='green' size={25} />EQUIP ITEM NFT</CardTitle>
                        {userID && <div className='text-black font-bold flex items-center gap-x-2'><Contact color='green' size={25} /> User ID: {userID}</div>}
                        {catId && <div className='text-black font-bold flex items-center gap-x-2'><Shapes color='green' size={25} /> CatNFT ID: {catId}</div>}
                        {itemId && <div className='text-black font-bold flex items-center gap-x-2'><Shapes color='green' size={25} /> ItemNFT ID: {itemId}</div>}
                        {isConnected && catId && itemId && userID && chainId === chain?.id && !isConfirmed && <div className="w-full flex items-center justify-center">
                            {isApproved ? <Button disabled={isWriteContracting || isConfirming} onClick={handleMintCatNFT} className={`${isConfirming ? 'w-full active:translate-y-1 bg-green-800 mt-3' : 'active:translate-y-1 bg-[#f68329] mt-3 hover:animate-none hover:bg-green-900 border-2 border-black text-black hover:text-white flex items-center justify-center shadow-lg w-fit duration-200'}`}>
                                {!isConfirming && <MousePointerClick className='mr-2' />}{isConfirming ? 'Confirming...' : 'EQUIP NOW'}
                            </Button> : <Button disabled={isWriteContractingApprove || isConfirmingApprove} onClick={handleApprove} className='w-fit active:translate-y-1 bg-green-800 mt-3'>
                                {isWriteContractingApprove || isConfirmingApprove ? "Approving..." : "Approve"}
                            </Button>}
                        </div>
                        }
                    </CardHeader>
