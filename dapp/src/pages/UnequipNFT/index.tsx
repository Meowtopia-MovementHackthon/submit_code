    ...
    return (
        <div className="relative w-full px-[1rem] pb-[2vh] min-h-screen flex flex-col items-center justify-start gap-y-[40px] bg-[url('@/assets/bg.png')] bg-cover bg-center bg-no-repeat font-Rubik">
            ...
            <div className="w-full flex flex-col gap-4 max-w-[500px]">
                ...
                {
                    isConfirmed && <div className='text-green-500 font-bold p-2 rounded-lg border-2 border-black shadow-lg flex items-center justify-center bg-white'><CircleCheckBig className='mr-2' />Unequip Item success. Check your item in Meowtopia</div>
                }
                <Card className='border-2 border-black shadow-lg'>
                    <CardHeader>
                        <CardTitle className='justify-center text-[16px] break-words flex items-center gap-x-2 mb-2'><ShieldPlus color='green' size={25} />UNEQUIP ITEM NFT</CardTitle>
                        ...
                        {isConnected && catId && itemId && userID && chainId === chain?.id && !isConfirmed && <div className="w-full flex items-center justify-center">
                            ...
                            {isApproved ? <Button disabled={isWriteContracting || isConfirming} onClick={handleMintCatNFT} className={`${isConfirming ? 'w-full active:translate-y-1 bg-green-800 mt-3' : 'active:translate-y-1 bg-[#f68329] mt-3 hover:animate-none hover:bg-green-900 border-2 border-black text-black hover:text-white flex items-center justify-center shadow-lg w-fit duration-200'}`}>
                                {!isConfirming && <MousePointerClick className='mr-2' />}{isConfirming ? 'Confirming...' : 'UNEQUIP'}
                            </Button> : <Button disabled={isWriteContracting || isConfirming} onClick={handleApprove} className='w-fit active:translate-y-1 bg-green-800 mt-3'>
                                Approve
                            </Button>}
                        </div>
                        }
