...
export class RedisService {
    ...
    */
    ...
    async iget<T = any, Z extends string | number = string | number>(key: string, id: Z, defaultValue?: T, config: { hasTTL: boolean } = { hasTTL: false }): Promise<T | undefined | null> {
    ...
    async lock(key: string, ttlInSecond: number): Promise<boolean> {
        let lockKey = await this.getLockKey(key)
    ...
    async unlock(key: string): Promise<boolean> {
        let lockKey = await this.getLockKey(key)
        let res = await this.redis.del(lockKey)
        if (res == 1) {
            return true
        }
