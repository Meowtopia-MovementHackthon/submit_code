...
const MintMaterialNFT = () => {
    ...
    useEffect(() => {
        console.log("🚀 ~ useEffect ~ tokensOfOwner:", tokensOfOwner)
    }, [tokensOfOwner])
    ...
    const handleClickMetamask = () => {
        if (userID && materialId && amount && chainId) {
            const url = `https://metamask.app.link/dapp/meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?materialId=${materialId}&amount=${amount}&userID=${userID}&chainId=${chainId}`
            window.open(url)
        }
    }
    ...
    const handClickOKX = () => {
        if (userID && materialId && amount && chainId) {
            const url = `https://meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?materialId=${materialId}&amount=${amount}&userID=${userID}&chainId=${chainId}`
    ...
    const handleMintNFT = async () => {
        ...
        writeContract({
            ...
            abi: MaterialNFTABI,
            functionName: 'mint',
            args: [address, materialId, amount, "0x"],
        });
    }
    ...
    const handleClickBinance = () => {
        if (userID && materialId && amount && chainId) {
            const url = encodeURIComponent(`https://meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?materialId=${materialId}&amount=${amount}&userID=${userID}&chainId=${chainId}`)
    ...
    return (
        <div className="relative w-full px-[1rem] pb-[2vh] min-h-screen flex flex-col items-center justify-start gap-y-[40px] bg-[url('@/assets/bg.png')] bg-cover bg-center bg-no-repeat font-Rubik">
            ...
            <div className="w-full flex flex-col gap-4 max-w-[500px]">
                ...
                {
                    isConfirmed && <div className='text-green-500 font-bold p-2 rounded-lg border-2 border-black shadow-lg flex items-center justify-center bg-white'><CircleCheckBig className='mr-2' />Mint Material NFT success. Check your item in Meowtopia</div>
                }
                <Card className='border-2 border-black shadow-lg'>
                    <CardHeader>
                        <CardTitle className='justify-center text-[16px] break-words flex items-center gap-x-2 mb-2'><ShieldPlus color='green' size={25} />MINT MATER NFT</CardTitle>
                        {userID && <div className='text-black font-bold flex items-center gap-x-2'><Contact color='green' size={25} /> User ID: {userID}</div>}
                        {/* {userAddress && <div className='text-black font-bold flex items-center gap-x-2'><Contact color='green' size={25} /> User Address: {userAddress}</div>} */}
                        ...
                        {amount && <div className='text-black font-bold flex items-center gap-x-2'><Shapes color='green' size={25} /> Amount: {amount}</div>}
                        {isConnected && materialId && amount && userID && chainId === chain?.id && !isConfirmed && <div className="w-full flex items-center justify-center">
                            <Button disabled={isWriteContracting || isConfirming} onClick={handleMintNFT} className={`${isConfirming ? 'w-full active:translate-y-1 bg-green-800 mt-3' : 'active:translate-y-1 bg-[#f68329] mt-3 hover:animate-none hover:bg-green-900 border-2 border-black text-black hover:text-white flex items-center justify-center shadow-lg w-fit duration-200'}`}>
...
export default MintMaterialNFT