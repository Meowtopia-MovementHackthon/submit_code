...
const UpgradeCatNFT = () => {
    ...
    const [catId1, setCatId1] = useState<string>(searchParams.get("catId1") || "")
    const [catId2, setCatId2] = useState<string>(searchParams.get("catId2") || "")
    ...
    const handleClickMetamask = () => {
        if (userID && catId1 && catId2 && chainId) {
            const url = `https://metamask.app.link/dapp/meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?catId1=${catId1}&catId2=${catId2}&&userID=${userID}&chainId=${chainId}`
            window.open(url)
        }
    }
    ...
    const handClickOKX = () => {
        if (userID && catId1 && catId2 && chainId) {
            const url = `https://meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?catId1=${catId1}&catId2=${catId2}&&userID=${userID}&chainId=${chainId}`
    ...
    const handleClickBinance = () => {
        if (userID && catId1 && catId2 && chainId) {
            const url = encodeURIComponent(`https://meowtopia-wallet-connect.vercel.app/upgrade-cat-nft?catId1=${catId1}&catId2=${catId2}&&userID=${userID}&chainId=${chainId}`)
        ...
        <div className="relative w-full px-[1rem] pb-[2vh] min-h-screen flex flex-col items-center justify-start gap-y-[40px] bg-[url('@/assets/bg.png')] bg-cover bg-center bg-no-repeat font-Rubik">
            ...
            <div className="w-full flex flex-col gap-4 max-w-[500px]">
                ...
                <Card className='border-2 border-black shadow-lg'>
                    <CardHeader>
                        <CardTitle className='justify-center text-[16px] break-words flex items-center gap-x-2 mb-2'><ShieldPlus color='green' size={25} />UPGRADE CAT NFT</CardTitle>
                        {userID && <div className='text-black font-bold flex items-center gap-x-2'><Contact color='green' size={25} /> User ID: {userID}</div>}
                        {catId1 && <div className='text-black font-bold flex items-center gap-x-2'><Shapes color='green' size={25} /> CatNFT ID 1: {catId1}</div>}
                        {catId2 && <div className='text-black font-bold flex items-center gap-x-2'><Shapes color='green' size={25} /> CatNFT ID 2: {catId2}</div>}
                        {isConnected && catId1 && catId2 && userID && chainId === chain?.id && !isConfirmed && <div className="w-full flex items-center justify-center">
